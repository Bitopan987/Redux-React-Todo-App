...
function App(props) {
  ...
  return (
    <>
      <h1 className="text-center text-6xl font-bold mb-10 text-indigo-900">
        Todo List
      </h1>
      <div className="bg-indigo-300 p-8 flex flex-col items-center mt-12 w-1/2 mx-auto rounded shadow-lg">
        <input
          ...
          className="w-5/6 p-2 rounded-md shadow-lg"
          onKeyUp={handleTodo}
        />
        <div className="w-full flex justify-center mt-10">
          <ul className="">
            {props.state &&
              props.state.map((elm, index) => {
                return (
                  <li
                    key={index}
                    className="flex justify-between w-96 bg-indigo-200 my-3 items-center text-2xl p-3 rounded shadow-lg"
                  >
                    <input
                      type="checkbox"
